...
public class BasicOverlayAudioArea extends AbstractFunctionalArea implements
	...
	// Worker Fields
	private OverlayWorker _worker;
	// Boolean Fields
	private boolean _canOverlay = true;
	// Reference Fields
	private AdvancedOverlayAudioArea _ao;

	public BasicOverlayAudioArea(AdvancedOverlayAudioArea ao) {
		super();
		_ao = ao;
	}
	...
	protected JPanel createAreaSpecific() {
		...
		// Set up area title
		JLabel areaTitle = new JLabel("Basic Overlay Audio");
		...
		_overlay = new JButton("Overlay");
		_overlay.setOpaque(false);
		_overlay.setFont(Defaults.DefaultButtonFont);
		_overlay.setAlignmentX(Component.CENTER_ALIGNMENT);
		_overlay.addActionListener(this);
		_overlay.setBackground(Defaults.DefaultGoButtonColour);
		// Create label
		...
		if (e.getSource().equals(_overlay)) {
			if (_canOverlay) {
				...
				if (_currentFile.getText().equals("No file selected")) {
					...
					if (_ao.getOutputName().trim().equals(
							"Enter in new output file name")) {
						outVideo = "output" + ext;
					} else {
						outVideo = _ao.getOutputName().trim() + ext;
					}
					if (!(_ao.getOutputLocation()
							.equals("No location selected"))) {
						outVideo = _ao.getOutputLocation() + "/" + outVideo;
					}
					...
					if (f.exists()){
						...
						if (selected == 0) {
							...
							_worker = new OverlayWorker(inVid, _currentFile.getText(),
									outVideo, this);
							_worker.execute();
						}
					}else{
	...
	public void processWorkerResults(int exitStatus) {
		// If non-zero, failure
		if (exitStatus == 9001){
			JOptionPane.showMessageDialog(null, "Overlaying audio successfully cancelled",
					"VAMIX Success", JOptionPane.INFORMATION_MESSAGE);
		}
	else if (exitStatus != 0) {
			JOptionPane.showMessageDialog(null,
					"Overlaying audio failed due to system error",
					"VAMIX Error", JOptionPane.ERROR_MESSAGE);
		} else {
			// Otherwise success
